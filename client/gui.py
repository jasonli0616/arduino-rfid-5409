...
class GUI(tkinter.Tk):
    ...
    def __draw_select_port(self):
        ...
        # Refresh
        refresh_button = ttk.Button(self, text="Refresh ports", command=self.__refresh_port_options)
        self.select_port_elements.append(refresh_button)

        # Submit to thread
        submit_button = ttk.Button(self, text="Submit port", command=self.__submit_port_in_thread)
    ...
    def __submit_port_in_thread(self):
    ...
    def __submit_port(self):
        ...
        try:
            ...
            messagebox.showerror("ERROR", "Port is invalid. Try refreshing or selecting a different port.")
    ...
    def __wait_for_uid(self, connection):
        ...
        # Add elements
        text = ttk.Label(self, text=f"Connected to {connection.port}.\nRestart program to connect to another port.")
        text.pack()
    ...
    def __ask_student_id(self, db, uid):
        ...
        submit = ttk.Button(self, text="Submit student ID", command=lambda: self.__create_student(db, uid, entry.get(), elements))
    ...
    def __create_student(self, db, uid, student_id, destroy_elements):
        if student_id:
            ...
            if db.get_uid(student_id):
                messagebox.showerror("ERROR", "ERROR: Student ID already exists in database.")
            else:
                db.create_new_user(uid, student_id)
                messagebox.showinfo("SUCCESS", "SUCCESS: User created. Scan again to use.")
